      ...
      <InputBoxStyle>
        ...
        <label>누구에게 보내실 건가요?</label>
          <SelectBoxStyle onChange={handleSelect} value={selected}> {
            selectList.map((item) => {
              return <SelectOptionStyle value={item.value} key={item.value}>
                {item.name}
              </SelectOptionStyle>
            })
          }
        ...
        <SectionStyle>
          <label>닉네임</label><InputNickNameStyle type='text' maxLength={20} value={nickname} onChange={onChangeNickName} placeholder='최대 20자까지 작성할 수 있습니다.' />
        </SectionStyle>
        <SectionStyle>
          <label>내용</label><TextareaContentStyle type='text' maxLength={100} value={contents} onChange={onChangeContents} placeholder='최대 100자까지 작성할 수 있습니다.' />
        </SectionStyle>
